...
public class MainStocksController(IRepositoryWrapper repo) : ControllerBase
{
    ...
    {
        bool usedInPurchase = await repo.PurchaseDetails.AnyAsync(x => x.ItemCode == code);
        if (usedInPurchase)
        {
            return ResponseHelper.Bad_Request(null,
                new DefaultResponseMessageModel("Cannot delete. This Main Stock is used in a Purchase record.", ""));
        }

        bool usedInPharmacyVoucher = await repo.PharmacyVoucherDetails.AnyAsync(x => x.ItemCode == code);
        if (usedInPharmacyVoucher)
        {
            return ResponseHelper.Bad_Request(null,
                new DefaultResponseMessageModel("Cannot delete. This Main Stock is used in a Pharmacy Voucher.", ""));
        }
