...
public class PacketTypesController(IRepositoryWrapper repo) : ControllerBase
{
    ...
    {
        bool usedInPurchase = await repo.PurchaseDetails.AnyAsync(x => x.TypeCode == typeCode);
        if (usedInPurchase)
        {
            return ResponseHelper.Bad_Request(null,
                new DefaultResponseMessageModel("Cannot delete. This Packet Type is used in a Purchase record.", ""));
        }

        bool usedInPharmacyVoucher = await repo.PharmacyVoucherDetails.AnyAsync(x => x.TypeCode == typeCode);
        if (usedInPharmacyVoucher)
        {
            return ResponseHelper.Bad_Request(null,
                new DefaultResponseMessageModel("Cannot delete. This Packet Type is used in a Pharmacy Voucher.", ""));
        }
